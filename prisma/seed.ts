...
async function main() {
  ...
  await prisma.siteSetting.create({ data: { siteTitle: "Dealer Portal", siteDescription: "B2B dealer ordering portal", notificationEmail: "admin@example.com", contactEmail: "admin@example.com", contactPhone: "(555) 000-0000", contactAddress: "123 Business Ave, Suite 100, Portland, OR 97201" } });

  await prisma.pageContent.upsert({ where: { pageKey: "dealer-settings" }, update: { payload: { showProductsToPublic: "true", showPricesToPublic: "false", allowSelfRegistration: "true", requireApprovalForRegistration: "true", requirePONumber: "false", adminNotificationEmail: "admin@example.com", shippingMethod: "flat", flatShippingRate: "15.00" } }, create: { pageKey: "dealer-settings", payload: { showProductsToPublic: "true", showPricesToPublic: "false", allowSelfRegistration: "true", requireApprovalForRegistration: "true", requirePONumber: "false", adminNotificationEmail: "admin@example.com", shippingMethod: "flat", flatShippingRate: "15.00" }, seo: {} } });

  await prisma.pageContent.upsert({ where: { pageKey: "home" }, update: { payload: { headline: "Quality Tools for Every Trade", subheadline: "Your trusted wholesale partner. Competitive dealer and distributor pricing on thousands of professional-grade products.", ctaText: "Browse Catalog", ctaHref: "/auth/login", ctaSectionTitle: "Ready to order?", ctaSectionBody: "Create a dealer account to access volume pricing and start placing orders today." } }, create: { pageKey: "home", payload: { headline: "Quality Tools for Every Trade", subheadline: "Your trusted wholesale partner. Competitive dealer and distributor pricing on thousands of professional-grade products.", ctaText: "Browse Catalog", ctaHref: "/auth/login", ctaSectionTitle: "Ready to order?", ctaSectionBody: "Create a dealer account to access volume pricing and start placing orders today." }, seo: { title: "Dealer Portal — Wholesale Tools & Supplies", description: "B2B ordering portal for dealers and distributors." } } });

  await prisma.pageContent.upsert({ where: { pageKey: "about" }, update: { payload: { title: "About Us", body: "Founded in 2010, we've grown from a small regional supplier to a trusted wholesale partner for hundreds of dealers and distributors across the country. Our mission is simple: provide professional-grade tools and supplies at competitive prices with reliable service.\n\nWe maintain strong relationships with leading manufacturers to ensure our dealers always have access to the products they need. Our dedicated support team is here to help with product selection, pricing, and order fulfillment." } }, create: { pageKey: "about", payload: { title: "About Us", body: "Founded in 2010, we've grown from a small regional supplier to a trusted wholesale partner for hundreds of dealers and distributors across the country." }, seo: {} } });

  // Home page features
  await prisma.pageGroupItem.createMany({ data: [
    { pageKey: "home", groupKey: "features", sortOrder: 0, payload: { icon: "🏷️", title: "Volume Pricing", description: "Exclusive dealer and distributor pricing tiers with automatic discounts based on your account level." } },
    { pageKey: "home", groupKey: "features", sortOrder: 1, payload: { icon: "📦", title: "Fast Fulfillment", description: "Same-day processing on orders placed before 2 PM. Real-time inventory and order tracking." } },
    { pageKey: "home", groupKey: "features", sortOrder: 2, payload: { icon: "🔧", title: "Pro-Grade Products", description: "Curated selection of professional-grade tools and supplies from trusted manufacturers." } },
  ]});
